...
The WebSocket server reads `PORT` and `HOST` from your environment (or `.env`) with defaults of `3055` and `localhost`. The provided `start-websocket` scripts respect the same values.
...
- Run the lightweight socket tests with `bun test` from `figma-mcp-server/`.

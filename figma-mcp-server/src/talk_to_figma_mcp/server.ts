...
const DEFAULT_WS_PORT = Number(process.env.PORT || 3055);
const isLocalServer = serverUrl === 'localhost' || serverUrl === '127.0.0.1' || serverUrl.startsWith('localhost:') || serverUrl.startsWith('127.0.0.1:');
const hasProtocol = serverUrl.startsWith('ws://') || serverUrl.startsWith('wss://');
const resolvedHost = hasProtocol ? serverUrl.replace(/^wss?:\/\//, '') : serverUrl;
const WS_URL = (() => {
  if (hasProtocol) {
    return serverUrl;
  }
  ...
  const hostHasPort = resolvedHost.includes(':');
  const portSegment = hostHasPort ? '' : `:${DEFAULT_WS_PORT}`;
  return `${protocol}://${resolvedHost}${portSegment}`;
})();
...
function processFigmaNodeResponse(result: unknown): any {
  ...
  if ("id" in resultObj && typeof resultObj.id === "string") {
    ...
    logger.info(
      `[Figma] [Get] Node processed: ${resultObj.name || "Unknown"} (ID: ${resultObj.id})`
    );

    if ("x" in resultObj && "y" in resultObj) {
      logger.debug(`[Figma] [Get] Node position: (${resultObj.x}, ${resultObj.y})`);
    }

    if ("width" in resultObj && "height" in resultObj) {
      logger.debug(`[Figma] [Get] Node dimensions: ${resultObj.width}×${resultObj.height}`);
    }
  }
...
function connectToFigma(port: number = DEFAULT_WS_PORT): Promise<void> {
  ...
  connectPromise = new Promise((resolve, reject) => {
    ...
    ws.on('open', async () => {
      ...
      currentChannel = null;
      reconnectTimer = null;
      const pendingJoin = lastJoinedChannel;
      connectPromise = null;
      resolve();

      if (pendingJoin) {
        try {
          await joinChannel(pendingJoin);
          logger.info(`Rejoined channel after reconnect: ${pendingJoin}`);
        } catch (err) {
          logger.warn(`Failed to rejoin channel ${pendingJoin}: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    });
    ...
    ws.on("message", (data: any) => {
      try {
        ...
        logger.debug(`[Figma] [Connection] Received message: ${JSON.stringify(myResponse)}`);
        logger.debug(`[Figma] [Connection] Raw payload: ${JSON.stringify(myResponse)}`);
        ...
        if (
          ...
          if (myResponse.error) {
            ...
            // Resolve even if result is falsy (empty array/object/0/false)
            if ("result" in myResponse) {
              request.resolve(myResponse.result);
            } else {
              // Fall back to resolving the whole response when result is absent
    ...
    ws.on('close', () => {
      ...
      if (!reconnectTimer) {
        ...
        reconnectTimer = setTimeout(() => {
          connectToFigma(port).catch((err) => logger.error(`Reconnect failed: ${err instanceof Error ? err.message : String(err)}`));
        }, 2000);
      }
    });
  });
...
async function sendCommandToFigma(
  ...
  if (requiresChannel && !currentChannel) {
    if (lastJoinedChannel) {
      await joinChannel(lastJoinedChannel);
    } else {
